...
// Every toggle writes this cookie, but until now nothing read it back on
// load, so the sidebar silently reset to `defaultOpen` on every reload
// regardless of what the user last chose. Read it once, synchronously, so
// there's no flash of the wrong state before it corrects itself.
...
function SidebarProvider({
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // biome-ignore lint: Cookie Store API is async and incompatible with this sync callback
